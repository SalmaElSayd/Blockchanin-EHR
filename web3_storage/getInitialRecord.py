...
def __main__(patient_id):
    ...
    transaction_receipt = w3.eth.get_transaction_receipt(all_transactions[0])
    ...
    dr_private_key = rsa.PrivateKey(int(os.getenv("N")), int(os.getenv( "E")), int(os.getenv("D")), int(os.getenv("P")), int(os.getenv("Q")))
    ...
    print(rec_details)
    if(len(rec_details) == 10 and int(rec_details[0]) == patient_id):
        print(print_init_record(rec_details))
        return print_init_record(rec_details)
